...
#RUN cp -f $HADOOP_HOME/share/hadoop/common/lib/guava-27.0-jre.jar $HIVE_HOME/lib/guava-19.0.jar

################################################################################
# install spark
RUN curl -s http://apache.claz.org/spark/spark-2.4.4/spark-2.4.4-bin-hadoop2.7.tgz | tar -xz -C /usr/local
RUN mv /usr/local/spark-2.4.4-bin-hadoop2.7 $SPARK_HOME
...
RUN touch -f $HADOOP_HOME/logs/fairscheduler-statedump.log
